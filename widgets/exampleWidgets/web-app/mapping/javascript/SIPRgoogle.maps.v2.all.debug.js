...
/*jslint continue: true, passfail: true, plusplus: true, maxerr: 50, indent: 4, unparam: true*/
/*global $, google, document, G_MAP_MARKER_SHADOW_PANE, console, setTimeout, clearTimeout, Image, LatLng, G_vmlCanvasManager, util, jc2cui*/
...
jc2cui.mapWidget.markerOverlay = (function () {
    ...
    // hoisted out of loop below to make jslint happy
    // doesnt like timeout function in loop
    function selectIt(placemark, doc, map) {
        var overlayId,
            featureId,
            item;
        ...
        // Use timeout as hack to ensure this happens after any polygons google maps determines are selected
        // markers win
        setTimeout(function () {
    ...
    function MarkerOverlay(map, doc) {
        ...
        this.resizeMapListener = google.maps.Event.addListener(this.map, 'resize', function () {
            var currentSize = map.getSize();
            if (Math.abs(that.lastSize.width - currentSize.width) > 2 || Math.abs(that.lastSize.height - currentSize.height) > 2) { // lots of false resize events in IE (e.g. mouse leave ANOTHER widget) makes resizing widget impossible if a few layers are loaded
                that.lastSize = currentSize;
                that.doRedraw();
            }
        });

        this.mouseOutMapListener = google.maps.Event.addListener(this.map, 'mouseout', function (e) {
            clearTimeout(that.mouseOverTimeout);
            if (that.toolTip !== null) {
                $('.markerCanvas', that.map.getContainer()).css('cursor', 'inherit');
                that.toolTip.remove();
            }
        });

        this.clickMapListener = google.maps.Event.addListener(this.map, 'click', function (overlay, latLng, overlayLatLng) {
            var loc = latLng || overlayLatLng,
                point,
                p,
                placemark;
            clearTimeout(that.mouseOverTimeout);
            ...
            if (!loc || loc.mouseClickHandled) {
                return;
            }

            point = that.map.fromLatLngToDivPixel(loc);
            ...
            for (p = (that.placemarks.length - 1); p >= 0; p--) {
                ...
                if ((placemark.hitRegion.minX <= point.x && placemark.hitRegion.maxX >= point.x) &&
                    ...
                    selectIt(placemark, doc); // hoisted out of loop to make jslint happy with timeout hack. See above
                    loc.mouseClickHandled = true;
                    break;
                }
            }
        });

        this.mouseMoveMapListener = google.maps.Event.addListener(this.map, 'mousemove', function (mapPoint) {
            var point,
                p,
                placemark;

            // endless mouse moves in IE
            if (mapPoint.equals(this.lastMouseMove)) {
                return;
            }
            this.lastMouseMove = mapPoint;
            ...
            that.mouseOverTimeout = setTimeout(function () {
                if (mapPoint.mouseMoveHandled) {
                    return;
                }

                point = that.map.fromLatLngToDivPixel(mapPoint);
                point.x = point.x - that.leftCanvas;
                point.y = point.y - that.topCanvas;
                ...
                for (p = (that.placemarks.length - 1); p >= 0; p--) {
                    ...
                    if ((placemark.hitRegion.minX <= point.x && placemark.hitRegion.maxX >= point.x) &&
                                    (placemark.hitRegion.minY <= point.y && placemark.hitRegion.maxY >= point.y)) {
                        $('.markerCanvas', that.map.getContainer()).css('cursor', 'pointer');
                        that.toolTip = $('<span class="placeMarkTooltip">' + placemark.name + '</span>')
                            .appendTo(that.map.getContainer())
                            .css({
                                'left': (point.x + 18).toString() + 'px',
                                'top': (point.y - 9).toString() + 'px'
                            });
                        mapPoint.mouseMoveHandled = true;
                        break;
                    }
                }
            }, 500);
    ...
    function init() {
        MarkerOverlay.prototype = new google.maps.Overlay();

        MarkerOverlay.prototype.initialize = function (map) {
            map.getPane(G_MAP_MARKER_SHADOW_PANE).appendChild(this.markerLayer[0]);
        };

        MarkerOverlay.prototype.removeFromMap = function () {
            if (this.ieMarkerOverlay) {
                this.ieMarkerOverlay.remove();
            } else {
                this.map.removeOverlay(this);
            }
        };

        MarkerOverlay.prototype.remove = function () {
            clearTimeout(this.mouseOverTimeout);
            if (this.toolTip) {
                $('.markerCanvas', this.map.getContainer()).css('cursor', 'inherit');
                this.toolTip.remove();
            }
            this.killCurrentDraw();
            google.maps.Event.removeListener(this.dragEndListener);
            google.maps.Event.removeListener(this.resizeMapListener);
        ...
        // Important?
        MarkerOverlay.prototype.copy = function () {
            var copy = new MarkerOverlay(this.doc, this.doc);
            copy.placemarks = this.placemarks.slice();
            return copy;
        };
        ...
        MarkerOverlay.prototype.killCurrentDraw = function (force) {
            ...
            clearTimeout(this.doDrawTimeout);
            this.canvas = null;
        };

        MarkerOverlay.prototype.doRedraw = function () {
            if (this.ieMarkerOverlay) {
                this.ieMarkerOverlay.doRedraw();
                return;
            }
            if (!this.markerLayer) {
                return;
            ...
            that.redrawTimeout = setTimeout(function () {
        ...
        MarkerOverlay.prototype.drawPlacemarks = function (canvas, context, bounds, start) {
            var i,
                block = 500,
                placemark,
                loc,
            ...
            for (i = 0; i < this.placemarks.length && i < start + block; i++) {
                ...
                if (this.canvas !== canvas) { // this draw was canceled
                    return;
                }
                ...
                loc = this.map.fromLatLngToDivPixel(this.placemarks[i].latLng);
                ...
                if (placemark.style && placemark.style.IconStyle && placemark.style.IconStyle.Icon && placemark.style.IconStyle.Icon.w) {
                    ...
                    if (placemark.name && placemark.name.length && drawLabels) {
                        if (noNativeCanvas) { // temp hack to to prevent excanavs bug where labels get drawn in random locations when offscreen - obviously not a real fix
                            if (!bounds.containsLatLng(this.placemarks[i].latLng)) {
                                continue;
                            }
                        }
            ...
            if (i < this.placemarks.length) {
                that = this;
                this.doDrawTimeout = setTimeout(function () {
                    that.drawPlacemarks(canvas, context, bounds, i);
                }, 10);//1);
            } else {
        ...
        // Redraw the MarkerOverlay based on the current projection and zoom level
        MarkerOverlay.prototype.redraw = function (force) {
            ...
            if (!force) { // on drag or pan draw gets called continuously
                this.doRedraw();
                return;
            }
            ...
            if (this.placemarks.length) {
                locContainer = this.map.fromLatLngToContainerPixel(this.placemarks[0].latLng);
                loc = this.map.fromLatLngToDivPixel(this.placemarks[0].latLng);
...
/*global $, google, document, G_MAP_MARKER_PANE, console, setTimeout, clearTimeout, util, jc2cui*/
...
jc2cui.mapWidget.markerOverlayIE = (function () {
    ...
    function MarkerOverlay(map, doc, placemarks) {
        ...
        this.container = $('<div />').click(function (event) {
    ...
    function init() {
        ...
        MarkerOverlay.prototype.initialize = function (map) {
            map.getPane(G_MAP_MARKER_PANE).appendChild(this.container[0]);
        };
        ...
        // Important?
        MarkerOverlay.prototype.copy = function () {
            var copy = new MarkerOverlay(this.map, this.doc);
            copy.placemarks = this.placemarks.slice();
            return copy;
        };
        ...
        MarkerOverlay.prototype.drawPlacemarks = function (markerLayer, start) {
            var markerMarkup = [],
                i,
                placemark,
                loc,
            ...
            if (i < this.placemarks.length) {
                this.drawPlacemarksTimer = setTimeout(function () {
                    that.drawPlacemarks(markerLayer, i);
                }, 10);
            } else {
        ...
        MarkerOverlay.prototype.redraw = function (force) {
            ...
            this.drawPlacemarksTimer = setTimeout(function () {
                that.drawPlacemarks(that.markerLayer);
            }, 10);
        };
    }
...
// - Speed up distanceBetweenPoints_

/**
* @name MarkerClustererPlus for Google Maps V3
* @version 2.0.15 [October 18, 2012]* @author Gary Little
* @fileoverview
...
function clusterInit() {
    function ClusterIcon(cluster, styles) {
        //cluster.getMarkerClusterer().extend(ClusterIcon, google.maps.Overlay);
        google.maps.Overlay.call(this);
    ...
    ClusterIcon.prototype = new google.maps.Overlay();
    ...
    ClusterIcon.prototype.initialize = function (map) {
        ...
        this.activeMap_.getPane(G_MAP_MARKER_PANE).appendChild(this.div_);

        // Fix for Issue 157
        google.maps.Event.addListener(this.activeMap_, "bounds_changed", function () {
        ...
        google.maps.Event.addDomListener(this.div_, "click", function (e) {
            ...
            if (!cDraggingMapByCluster) {
				...
				*/
                ...
                // Doesnt seem neccesary to fire click. Map is firing click
                //google.maps.Event.trigger(mc, "click", cClusterIcon.center_);
                ...
                if (mc.getZoomOnClick()) {
                    ...
                    setTimeout(function () {
                        ...
                        if (mz !== null && (mc.activeMap_.getZoom() > mz)) {
                            mc.activeMap_.setZoom(mz + 1);
                        }
                    }, 100);
                }
    ...
    * Draws the icon.
    */
    ClusterIcon.prototype.redraw = function (force) {
        // TBD - Only do if force?
        if (this.visible_) {
	...
	*/
    ClusterIcon.prototype.show = function (posMayHaveChanged) {
        if (this.div_) {
            if (posMayHaveChanged || !this.div_.style.cssText.length || this.div_.style.display === "none") {
                var pos = this.getPosFromLatLng_(this.center_);
	...
	*/
    ClusterIcon.prototype.getPosFromLatLng_ = function (latlng) {
        var pos = this.activeMap_.fromLatLngToDivPixel(latlng);
    ...
    */
    Cluster.prototype.isMarkerInClusterBounds = function (marker) {
        ...
        return this.markerClusterer_.isMarkerInBounds_(marker, this.bounds_);
    };
    ...
    */
    ...
    MarkerClusterer.prototype = new google.maps.Overlay();

    // ClusterManager is using v3 style setMap - only for remove but implementing all
    MarkerClusterer.prototype.setMap = function (map) {
        if (this.activeMap_ === map) {
            return;
        }
        if (this.activeMap_) {
            this.activeMap_.removeOverlay(this);
        }
        if (map) {
            map.addOverlay(this);
        }
    ...
    MarkerClusterer.prototype.regenerate = function () {
        clearTimeout(this.regenerateTimeout);
        var that = this;
        this.regenerateTimeout = setTimeout(function () {
            that.resetViewport_(false);
            that.redraw_();

            // Why would this ever happen?
            for (i = 0; i < that.markers_.length; i++) {
                marker = that.markers_[i];
                if (!marker.cluster) {
                    marker.setMap(null);
                }
            }
    ...
    MarkerClusterer.prototype.initialize = function (map) {
        this.activeMap_ = map;
        ...
        this.listeners_ = [
            google.maps.Event.addListener(this.activeMap_, "zoomend", function (oldLevel, newLevel) {
                cMarkerClusterer.regenerate();
            }),
            google.maps.Event.addListener(this.activeMap_, "moveend", function () {
                // often get bogus moveend events - at least in IE
                var newCenter = cMarkerClusterer.activeMap_.getCenter();
                if (!newCenter.equals(cMarkerClusterer.lastCenter)) {
                    cMarkerClusterer.lastCenter = newCenter;
                    cMarkerClusterer.regenerate();
                }
            })
        ];
    ...
    MarkerClusterer.prototype.redraw = function (force) { };
    ...
    */
    MarkerClusterer.prototype.fitMapToMarkers = function () {
        ...
        jc2cui.mapWidget.fitToBounds(bounds);
    };
    ...
    */
    MarkerClusterer.prototype.getExtendedBounds = function (bounds) {
        ...
        var trPix = this.activeMap_.fromLatLngToDivPixel(tr);
        trPix.x += this.gridSize_;
        trPix.y -= this.gridSize_;

        var blPix = this.activeMap_.fromLatLngToDivPixel(bl);
        blPix.x -= this.gridSize_;
        blPix.y += this.gridSize_;

        /* Convert the pixel points back to LatLng
        var ne = this.activeMap_.fromDivPixelToLatLng(trPix);
        var sw = this.activeMap_.fromDivPixelToLatLng(blPix);
    ...
    */
    MarkerClusterer.prototype.isMarkerInBounds_ = function (marker, bounds) {
        ...
        var markerPix = this.activeMap_.fromLatLngToDivPixel(marker.getPosition());

        return markerPix.x > bounds.sw.x && markerPix.x < bounds.ne.x &&
               markerPix.y > bounds.ne.y && markerPix.y < bounds.sw.y;
    };
    ...
    */
    MarkerClusterer.prototype.addToClosestCluster_ = function (marker) {
        ...
        for (i = 0; i < this.clusters_.length; i++) {
            ...
            if ((!this.attributes_ || cluster.doMarkerAttributesMatch(marker)) && cluster.isMarkerInClusterBounds(marker)) { // dont consider if using attributes and they dont match
    ...
    */
    MarkerClusterer.prototype.createClusters_ = function (keys, iFirst) {
        ...
        if (this.activeMap_.getZoom() > 3) {
            mapBounds = new google.maps.LatLngBounds(this.activeMap_.getBounds().getSouthWest(),
              this.activeMap_.getBounds().getNorthEast());
        } /*else {
        ...
        for (i = iFirst; i < iLast; i++) {
            ...
            if (this.markers_.hasOwnProperty(key)) {
                ...
                if (!marker.cluster &&
                    ...
                    (!bounds || this.isMarkerInBounds_(marker, bounds))) {
...
/**
    ...
    *  different tooltip for each cluster marker.)* @property {function} [calculator=MarkerClusterer.CALCULATOR] The function used to determine
...
function MarkerClusterer(map, opt_markers, opt_options) {
    // MarkerClusterer implements google.maps.Overlay interface. We use the
    // extend function to extend MarkerClusterer with google.maps.Overlay
    // because it might not always be available when the code is defined so we
    // look for it at the last possible moment. If it doesn't exist now then
    // there is no point going ahead :)
    //this.extend(MarkerClusterer, google.maps.Overlay);
    google.maps.Overlay.call(this);
    ...
    map.addOverlay(this);
}/*jslint continue: true, passfail: true, plusplus: true, maxerr: 50, indent: 4, nomen: true, unparam: true*/
...
/*global google, $, console, util, jc2cui*/

util.namespace('jc2cui.mapWidget'); // var jc2cui = jc2cui || {}; no longer passes JSlint :(
jc2cui.mapWidget.latLonControl = (function () {
    "use strict";

    function LatLonControl() {
        return this;
    }

    return {
        init: function () {
            LatLonControl.prototype = new google.maps.Control(true, false);

            LatLonControl.prototype.printable = function () {
                return true;
            };

            LatLonControl.prototype.selectable = function () {
                return false;
            };

            LatLonControl.prototype.getDefaultPosition = function () {
                return new google.maps.ControlPosition(google.maps.ANCHOR_BOTTOM_RIGHT, new google.maps.Size(0, 14));
            };

            LatLonControl.prototype.initialize = function (map) {
                var that = this,
                    node = $('<div class="customMapControl"><span id="latLon-control"></span><div class="controlBack"></div></div>');
                map.getContainer().appendChild(node[0]);
                $('.controlBack', node).css({ opacity: 0.3 }); // cross browser
                this.control = $('#latLon-control', node)[0];
                google.maps.Event.addListener(map, 'mousemove', function (latLon) {
                    that.updatePosition(latLon);
                });
                return node[0];
            };
...
/*global google: true, window, document, $, console, G_DEFAULT_ICON, setTimeout, clearTimeout, GFusionMap, GSmallMapControl, GMapTypeControl, GScaleControl, GKeyboardHandler, clusterInit, G_MAP_MAP_PANE, addArcGISMapType, util, jc2cui*/

var OWF;

function googleLoaded() { // ajax load
    "use strict";
    jc2cui.mapWidget.renderer.googleReady();
}

function mapsLoaded() { // direct load
    "use strict";
...
jc2cui.mapWidget.renderer = (function () {
    ...
    function makeFeatureEditable(feature, editable) {
        if (feature.data.type === 'marker') {
            if (editable) {
                feature.mapObject.enableDragging();
            } else {
                feature.mapObject.disableDragging();
            }
        } else {
            if (editable) {
                feature.mapObject.enableEditing();
            } else {
                feature.mapObject.disableEditing();
            }
        }
    }
    ...
    Overlay.prototype.setVisibility = function (visibility) {
        ...
        $.each(this.features, function (index, feature) {
            if (visibility) {
                feature.mapObject.show();
                if (drawingTools.is(":visible")) {
                    makeFeatureEditable(feature, true);
                }
            } else {
                feature.mapObject.hide();
                if (drawingTools.is(":visible")) {
    ...
    Overlay.prototype.remove = function () {
        ...
        $.each(this.features, function (index, feature) {
            map.removeOverlay(feature.mapObject);
            if (jc2cui.mapWidget.infoWindow.getMapItem() && jc2cui.mapWidget.infoWindow.getMapItem().id === feature.data.id) {
                jc2cui.mapWidget.infoWindow.close();
            }
        });
    ...
    function getFeatureBounds(feature) {
        ...
        case 'marker':
            ...
            bounds.extend(feature.mapObject.getLatLng());
            break;
        case 'line':
        case 'shape':
            bounds = feature.mapObject.getBounds();
            break;
        }
        return bounds;
    }
    ...
    Overlay.prototype.bounds = function () {
        ...
        $.each(this.features, function (index, feature) {
            ...
            if (b && !b.isEmpty()) {
                if (bounds.isEmpty()) { // extend gets confused if extend a bounds that wraps the whole world into an empty bounds 
                    bounds = b;
                } else {
    ...
    // While maps API v2 has getKML function on overlays, our kml contains more data (id for example!)
    ...
    function showEditBalloon(feature) {
        jc2cui.mapWidget.showUserDrawnEditWindow(feature.data, {
            ...
            updated: function (updatedProps) {
                var newMarker,
                    kml = generateKml(updatedProps);

                switch (feature.data.type) {
                case 'marker':
                    // setImage not recommended see API docs - also no setTitle
                    // blow it away and recreate
                    newMarker = new google.maps.Marker(updatedProps.coordinates[0], {
                        icon: new google.maps.Icon(feature.mapObject.getIcon(), updatedProps.iconUrl),
                        title: updatedProps.title,
                        draggable: true
                    });

                    // But need to recreate various listeners on new marker - bad
                    google.maps.Event.addListener(newMarker, "dragend", function (latLng) {
                        feature.data.coordinates[0] = latLng;
                        kml = generateKml(feature.data);
                        jc2cui.mapWidget.publishDrawing(kml, feature.data.id, feature.data.title, !feature.mapObject.isHidden());
                    });

                    map.addOverlay(newMarker);
                    map.removeOverlay(feature.mapObject);
                    feature.mapObject = newMarker;
                    addEditableFeature(feature); // add back click listener
                    break;
                case 'line':
                    feature.mapObject.setStrokeStyle({
                        color: updatedProps.strokeColor,
                        weight: updatedProps.strokeWeight,
                        opacity: updatedProps.strokeOpacity
                    });
                    break;
                case 'shape':
                    feature.mapObject.setStrokeStyle({
                    ...
                    feature.mapObject.setFillStyle({
                        color: updatedProps.fillColor,
                        opacity: updatedProps.fillOpacity
                    });
                    break;
                }
                ...
                /* No good - our KML has much more data
                feature.mapObject.getKml(function (kml) { 
                jc2cui.mapWidget.publishDrawing(kml, feature.data.id);
                });*/
    ...
    addEditableFeature = function (feature) {
        ...
        google.maps.Event.addListener(feature.mapObject, 'click', function (latLng) {
            if (drawingTools.is(":visible")) {
    ...
    function addPlacemark(event) {
        var body = $('body'),
            mapDiv = $(map.getContainer()),
            placementMarker = $('<img class="placementmarker" src="' + jc2cui.mapWidget.getDefaultDrawingIcon() + '">').appendTo(body),
            tooltip,
            mouseMoveListener,
            clickListener,
        ...
        placementMarker.css({
            'left': (event.pageX - placementMarker.width() / 2).toString() + 'px',
            'top': (event.pageY - placementMarker.height() - 1).toString() + 'px'
        });

        if (placemarkTooltipShows++ < 3) {
            tooltip = $('<span class="tooltip">Click to place me on the map</span>')
                .appendTo(body)
                .css({
                    'left': (event.pageX + 18).toString() + 'px',
                    'top': (event.pageY - 9).toString() + 'px'
                });
        }

        mouseMoveListener = function (e) {
            if (e.pageX < mapDiv.position().left) {
                return;
            }
            placementMarker.css({
                'left': (e.pageX - placementMarker.width() / 2).toString() + 'px',
                'top': (e.pageY - placementMarker.height() - 1).toString() + 'px'
            });

            if (tooltip) {
                tooltip.css({
                    'left': (e.pageX + 18).toString() + 'px',
                    'top': (e.pageY - 9).toString() + 'px'
                });
            }
        };
        body.bind('mousemove', mouseMoveListener);

        clickListener = google.maps.Event.addListener(map, 'click', function (overlay, latLng, overlayLatLng) {
            var title = 'marker ' + (++numberDrawn.marker),
                data = { type: 'marker', id: id, coordinates: [latLng || overlayLatLng], title: title, iconUrl: jc2cui.mapWidget.getDefaultDrawingIcon() },
                feature = { data: data },
                kml;

            if (!defaultMarkerImage) {
                defaultMarkerImage = new google.maps.Icon(null, data.iconUrl);
                defaultMarkerImage.iconAnchor = new google.maps.Point(16, 32);
                defaultMarkerImage.iconSize = new google.maps.Size(32, 32);
                defaultMarkerImage.shadow = "";
            }

            feature.mapObject = new google.maps.Marker(data.coordinates[0], {
                icon: defaultMarkerImage,
                title: data.title,
                draggable: true
            });

            feature.getBounds = function () {
                var bounds = new google.maps.LatLngBounds();
                feature.mapObject.extendBounds(bounds); // non standard extension method
                return bounds;
            };
            data.getBounds = feature.getBounds;

            map.addOverlay(feature.mapObject);

            google.maps.Event.addListener(feature.mapObject, "dragend", function (latLng) {
                data.coordinates[0] = latLng;
                //feature.data === data - no need to update
        ...
        endDrawingCallback = function () {
            placementMarker.remove();
            if (tooltip) {
                tooltip.remove();
            }
            body.unbind('mousemove', mouseMoveListener);
            google.maps.Event.removeListener(clickListener);
        };
    }

    function addPoly(event, closed) {
        ...
        if (closed) {
            poly = new google.maps.Polygon([], strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity);
        } else {
            poly = new google.maps.Polyline([], strokeColor, strokeWeight, strokeOpacity);
        }

        map.addOverlay(poly);
        poly.enableDrawing();

        if ((closed && polyTooltipShows++ < 3) || (!closed && lineTooltipShows++ < 3)) {
            tooltipText = 'Click to start drawing a ' + (closed ? 'shape' : 'line');
            tooltip = $('<span class="tooltip">' + tooltipText + '</span>')
                .appendTo(body)
                .css({
                    'left': (event.pageX + 18).toString() + 'px',
                    'top': (event.pageY - 9).toString() + 'px'
        ...
        if (tooltip) {
            ...
            leftClickListener = google.maps.Event.addListener(map, 'click', function (overlay, latLng, overlayLatLng) {
                if (poly.getVertexCount() === 1 || (closed && poly.getVertexCount() === 2)) {
                    tooltipText = 'Click to continue drawing a ';
                } else {
                    tooltipText = 'Click first or last point to end this ';
                }
                tooltipText += closed ? 'shape' : 'line';
                tooltip.html(tooltipText);
            });
        }

        google.maps.Event.addListener(poly, "endline", function () {
            var title = (closed ? 'shape ' : 'line ') + (++numberDrawn[closed ? 'shape' : 'line']),
                data = {
                    type: closed ? 'shape' : 'line',
                    id: id,
                    coordinates: [],
                    title: title,
                    iconUrl: [jc2cui.mapWidget.getAppPath(), 'images/', closed ? 'poly' : 'line', 'Mask.png"'].join(''),
            ...
            for (i = 0; i < poly.getVertexCount(); i++) {
                data.coordinates.push(poly.getVertex(i));
            }
            ...
            setTimeout(function () {
                makeFeatureEditable(feature, true); // cant be in endline callback
            }, 500);
            ...
            google.maps.Event.addListener(poly, "lineupdated", function () {
                ...
                var updatedKml = generateKml(data);
                jc2cui.mapWidget.publishDrawing(updatedKml, feature.data.id, feature.data.title, !feature.mapObject.isHidden());
            });
        ...
        google.maps.Event.addListener(poly, "cancelline", function () {
            setDrawingMode(0);
        });
        ...
        endDrawingCallback = function () {
            ...
            if (!completed) {
                poly.disableEditing(); // or remove has no effect - new google behavior 
                map.removeOverlay(poly);
            }
        };
    }

    setDrawingMode = function (mode, event) {
        var urlPrefix = 'images/',
            polyImageUrl = urlPrefix;

        // set buttons 
        selectButton.attr('src', mode === 0 ? urlPrefix + 'Bsd.png' : urlPrefix + 'Bsu.png');
        placemarkButton.attr('src', mode === 1 ? urlPrefix + 'Bmd.png' : urlPrefix + 'Bmu.png');
        if (mode === 2) {
            polyImageUrl += 'Bldm.png'; // drawing lines
        } else if (mode === 3) {
            polyImageUrl += 'Bpdm.png'; // drawing shapes
        } else {
            polyImageUrl += polyType === 2 ? 'Blum.png' : 'Bpum.png'; // not drawing polys
        }
        polyButton.attr('src', polyImageUrl);
        polyTypeDiv.hide();

        // end any drawing in progress.
        if (endDrawingCallback) {
            endDrawingCallback();
            //map.getDragObject().setDraggableCursor('hand'); - not working very well
            endDrawingCallback = null;
        }

        // start drawing
        if (event && mode) {
            //map.getDragObject().setDraggableCursor('crosshair'); - reset back to hand often fails!
            switch (mode) {
            case 1:
                addPlacemark(event);
                break;
    ...
    function setDrawingToolsVisibility(show) {
        if (show) {
            if (!drawingTools) {
                drawingTools = $('<div id="drawingtools"><img id="select" title="Select/edit map features"><img id="placemark" title="Add a placemark"><span><img id="poly"><span id="polydrop"></span></span><div id="polytype" class="menu"><div id="line" class="menuitem"><img src="images/Ml.png">&nbsp;Draw a line</div><div id="shape" class="menuitem"><img src="images/Mp.png">&nbsp;Draw a shape</div></div>')
                    .appendTo($(map.getContainer()));

                selectButton = $('#select', drawingTools).click(function (event) {
                    setDrawingMode(0, event);
                });
                placemarkButton = $('#placemark', drawingTools).click(function (event) {
                    setDrawingMode(1, event);
                });
                polyButton = $('#poly', drawingTools).click(function (event) {
                    setDrawingMode(polyType, event);
                });
                polyTypeDiv = $('#polytype', drawingTools);
                $('#polydrop', drawingTools).click(function (event) {
                    polyTypeDiv.show();
                });
                $('#line', polyTypeDiv).hover(function () {
                    $(this).addClass("selectedmenuitem");
                }, function () {
                    $(this).removeClass("selectedmenuitem");
                }).click(function (event) {
                    polyType = 2;
                    setDrawingMode(2, event);
                });
                $('#shape', polyTypeDiv).hover(function () {
                    $(this).addClass("selectedmenuitem");
                }, function () {
                    $(this).removeClass("selectedmenuitem");
                }).click(function (event) {
                    polyType = 3;
                    setDrawingMode(3, event);
                });
                drawingTools.mouseleave(function (event) {
                    polyTypeDiv.hide();
                });
                setDrawingMode(0);
            } else {
    ...
    function getViewStatus() {
        ...
        return jc2cui.mapWidget.createViewStatus(bounds.getSouthWest().lat(), bounds.getSouthWest().lng(), bounds.getNorthEast().lat(), bounds.getNorthEast().lng(), center.lat(), center.lng(), range);
    }

    // For Maps API marker implementation (not just markers - polys too)
    function processPlacemarkPart(placemarkPart, placemark, doc) {
        map.addOverlay(placemarkPart);
        if (!placemark.visible) {
            placemarkPart.hide();
        }
    ...
    function initMaps() {
        ...
        if (config.fusionMap) {
            map = new GFusionMap(document.getElementById("map"), {
                isAuthenticated: false,
                layers: config.fusionMap.layers,
                serverUrl: config.fusionMap.serverUrl,
                useGoogleLayers: false
            }, { logoPassive: true });

            for (i = 0; i < config.fusionMap.arcGISLayers; i++) {
                addArcGISMapType(config.fusionMap.arcGISLayers[i].url, 0, 15, config.fusionMap.arcGISLayers[i].name, true, 1024, 512, 512, map);
            }
        } else {
            map = new google.maps.Map2(document.getElementById("map"), { logoPassive: true });
        }
        map.setCenter(new google.maps.LatLng(39.5, -98.3), 3);

        if (typeof map.getDefaultUI === "function") {
            uiOptions = map.getDefaultUI();
            uiOptions.controls.scalecontrol = true; // users always want scale control - could add UI control later
            map.setUI(uiOptions);
        } else { // old versions of maps API on SIPR dont support getDefault UI
        ...
        nativeMapTypes = map.getMapTypes().slice();
        lastNativeMap = map.getCurrentMapType();
        ...
        $(map.getPane(G_MAP_MAP_PANE)).parent().mousedown(function (e) {
        ...
        mapViewChangeCallback = function () {
        ...
        google.maps.Event.addListener(map, 'zoomend', mapViewChangeCallback);
        ...
        google.maps.Event.addListener(map, 'maptypechanged', function () {
            ...
            $.each(wmsBackgroundDocs, function (index, wmsDoc) {
                if (wmsDoc.features[0].mapType !== currentMapType) {
                    if (wmsDoc.visible) {
            ...
            if ($.inArray(currentMapType, nativeMapTypes) > -1) {
                lastNativeMap = currentMapType;
            }
        });
        ...
        MarkerDummy.prototype.getLatLng = function () {
            return this.latLng;
        };
        ...
        MarkerDummy.prototype.setMap = function (map) {
            if (map) {
                if (!this.placemark.addedToMarkerOverlay) {
                    this.markerOverlay.addMarker(this.placemark, this.getLatLng());
                }
                this.markerOverlay.placemarkVisibilityChanged(this.placemark);
        ...
        // Hack to support opacity on ground overlays. See also http://code.google.com/p/projectedoverlay/
        // but that assumes non mercator tranformed image. Seems more likely that kml intended for Earth
        // will already be transformed
        // Doesnt handle rotation
        function DimmableGroundOverlay(url, bounds, map, opacity) {
            this.url = url;
            this.bounds = bounds;
            this.map = map;
            this.opacity = opacity;
            this.groundOverlay = new google.maps.GroundOverlay(url, bounds);
            this.map.addOverlay(this.groundOverlay);
            this.trySetOpacityAttempts = 0;
            this.setOpacity();

            var that = this;
            google.maps.Event.addListener(map, 'zoomend', function () {
                if (!that.isHidden()) {
                    that.trySetOpacityAttempts = 0;
                    that.setOpacity();
                }
            });
        }

        DimmableGroundOverlay.prototype.extendBounds = function (bounds) {
            if (this.bounds && !this.bounds.isEmpty()) {
                bounds.extend(this.bounds.getSouthWest());
                bounds.extend(this.bounds.getNorthEast());
            }
        };

        DimmableGroundOverlay.prototype.setOpacity = function () {
            // Dont bother if we've already been removed
            if (!this.map || this.groundOverlay.isHidden()) {
                return;
            }
            ...
            var that = this,
                it = $("img[src$='" + this.url + "']");
            if (it.length) {
                it.css('opacity', this.opacity);
            } else {
                if (++this.trySetOpacityAttempts < 60) {
                    setTimeout(function () {
                        that.setOpacity();
                    }, 1000);
                }
        ...
        DimmableGroundOverlay.prototype.removeFromMap = function () {
            this.map.removeOverlay(this.groundOverlay);
        };

        DimmableGroundOverlay.prototype.isHidden = function () {
            return this.groundOverlay.isHidden();
        };

        DimmableGroundOverlay.prototype.setVisible = function (visible) {
            if (visible) {
                this.groundOverlay.show();
                this.trySetOpacityAttempts = 0;
                this.setOpacity();
            } else {
                this.groundOverlay.hide();
            }
        };

        DimmableGroundOverlay.prototype.refreshIcon = function (groundOverlay) {
            this.remove();
        ...
        kmlParserSettings = {
            ...
            containerRemoved: function (container) {
                // TBD - one or the other
                if (container.markerOverlay) {
                    container.markerOverlay.removeFromMap();
                }
                jc2cui.mapWidget.clusterManager.cluster(false, container);
            },
            ...
            isRegionActive: function (region) { // TBD - Move a bit more into the parser
                ...
                sw = map.fromLatLngToDivPixel(region.latLonAltBox.getSouthWest());
                ne = map.fromLatLngToDivPixel(region.latLonAltBox.getNorthEast());
            ...
            formatView: function (format) { // TBD - Move a bit more into the parser
                ...
                format = format.replace('[horizFov]', ((ne.distanceFrom(nw) / 2) / range));
                format = format.replace('[vertFov]', ((sw.distanceFrom(nw) / 2) / range));
                format = format.replace('[horizPixels]', map.getSize().width);
                format = format.replace('[vertPixels]', map.getSize().height);
            ...
            polyline: function (placemark, path, doc) {
                ...
                polyline = new google.maps.Polyline(polylineOptions.path, polylineOptions.strokeColor, polylineOptions.strokeWeight, polylineOptions.strokeOpacity);
            ...
            polygon: function (placemark, paths, doc) {
                ...
                for (i = 0; i < paths.length; i++) {
                    ...
                    polygon = new google.maps.Polygon(coords, polygonOptions.strokeColor, polygonOptions.strokeWeight, polygonOptions.strokeOpacity, polygonOptions.fillColor, polygonOptions.fillOpacity);
                    processPlacemarkPart(polygon, placemark, doc);
                    polygons.push(polygon);
                }
                return polygons;
            },
            // This native google maps 2 implementation cant rotate - ground overlay latlonbox can include rotate
            ...
            groundOverlay: function (groundOverlay, doc) {
                ...
                dgo = new DimmableGroundOverlay(groundOverlay.icon.getHref(), bounds, map, groundOverlay.opacity);
                if (!groundOverlay.visible) {
                    dgo.setVisible(false);
                }
                return dgo;
            },
            bounds: function () {
        ...
        google.maps.Marker.prototype.setVisible = function (visibility) {
            if (visibility) {
                this.show();
            } else {
                this.hide();
        ...
        google.maps.Polyline.prototype.extendBounds = function (bounds) {
            ...
            if (thisBounds && !thisBounds.isEmpty()) {
        ...
        google.maps.Polyline.prototype.setVisible = google.maps.Marker.prototype.setVisible;
        ...
        google.maps.Polygon.prototype.setVisible = google.maps.Marker.prototype.setVisible;
        ...
        cssUrl += 'css/google.maps.v2.all.min.css';

        $('<link rel="stylesheet" type="text/css" href="' + cssUrl + '" >').appendTo("head");

        clusterInit();

        jc2cui.mapWidget.latLonControl.init();
        map.addControl(new jc2cui.mapWidget.latLonControl.Control());
    ...
    function centerOnBounds(bounds, zoom) {
        ...
        if (!bounds.isEmpty()) {
            if (zoom === 'auto') {
                ...
                centerOnLocation(bounds.getCenter());
            } else {
    ...
    function setView(item, zoom) {
        ...
        if (item.viewer) {
            ...
            if (!bounds.isEmpty() && !map.getBounds().equals(bounds)) {
                centerOnBounds(bounds, zoom);
            }
        }
    }

    function createWmsTileLayer(url, params, version3) {
        var tile = new google.maps.TileLayer(new google.maps.CopyrightCollection(''), 1, 17);
        tile.baseUrl = url;
        tile.params = params;
        tile.getTileUrl = function (point, zoom) {
            var ulp = new google.maps.Point(point.x * 256, (point.y + 1) * 256),
                lrp = new google.maps.Point((point.x + 1) * 256, point.y * 256),
                ul = google.maps.NORMAL_MAP.getProjection().fromPixelToLatLng(ulp, zoom),
                lr = google.maps.NORMAL_MAP.getProjection().fromPixelToLatLng(lrp, zoom),
                bbox,
                postParams = '',
                first = url.indexOf('?') < 0;
    ...
    // TBD - inherit from feature in KMLParser instead of duplicating much of it here
    function WmsFeature(layer) {
        this.layer = layer;
        this.features = [];
    }

    WmsFeature.prototype.remove = function () {
        map.removeOverlay(this.layer);
    };

    WmsFeature.prototype.extendBounds = function (bounds) {
        return;
    };

    WmsFeature.prototype.getAllVisiblePlacemarks = function (markers) {
        return markers || [];
    };

    WmsFeature.prototype.setVisible = function (visibility) {
        if (visibility) {
            this.layer.show();
        } else {
            this.layer.hide();
    ...
    // TBD - inherit from feature in KMLParser instead of duplicating much of it here
    function WmsBackgroundFeature(mapType) {
        this.mapType = mapType;
        this.features = [];
    }

    WmsBackgroundFeature.prototype.remove = function () {
        if (map.getCurrentMapType() === this.mapType) {
            map.setMapType(lastNativeMap);
        }
        map.removeMapType(this.mapType);
    };

    WmsBackgroundFeature.prototype.extendBounds = function (bounds) {
        return;
    };

    WmsBackgroundFeature.prototype.getAllVisiblePlacemarks = function (markers) {
        return markers || [];
    };

    WmsBackgroundFeature.prototype.setVisible = function (visibility) {
        if (visibility) {
            if (map.getCurrentMapType() !== this.mapType) {
    ...
    function WmsDoc(id) {
        ...
        this.id = id;
        wmsBackgroundDocs[id] = this;
    }
    WmsDoc.prototype = new jc2cui.mapWidget.kmlParser.Container(null, true);
    baseRemove = WmsDoc.prototype.remove;
    WmsDoc.prototype.remove = function () {
        delete wmsBackgroundDocs[this.id];
        baseRemove.call(this); // after removing it from wmsBackgroundDocs or tree visibility checkbox may be wrong on reload
    };
    ...
    function loadWms(overlayId, featureId, name, url, params, background, sender, msg) {
        ...
        layer = createWmsTileLayer(url, params, version3);

        if (background) {
            customMapType = new google.maps.MapType([layer], google.maps.NORMAL_MAP.getProjection(), name);
            map.addMapType(customMapType);

            // If small map and menu type showing apparent bug forces us to cause refresh or control doesnt display
            if (typeof map.setUIToDefault === "function") {
                map.setUIToDefault();
            } else {
                map.addControl(new GMapTypeControl());
            }
            ...
            doc = new WmsDoc(overlayId + featureId);
            doc.features.push(new WmsBackgroundFeature(customMapType));
            doc.setVisible(visible, true);
        } else {
            tileLayerOverlay = new google.maps.TileLayerOverlay(layer);
            map.addOverlay(tileLayerOverlay);
            doc = new jc2cui.mapWidget.kmlParser.Container();
            doc.features.push(new WmsFeature(tileLayerOverlay));
            doc.setVisible(visible, true);
        }
    ...
    function removeFeature(data, sender, msg) {
        var overlay = overlays[data.overlayId],
            ...
            oldDoc;
        if (!overlay) {
            jc2cui.mapWidget.error({ sender: sender ? sender.id : '', type: 'map.feature.unplot', msg: msg, error: 'Unable to find overlay ' + data.overlayId || data.url });
            return;
        }

        //first try user drawn
        if (overlay.features[data.featureId]) {
            map.removeOverlay(overlay.features[data.featureId].mapObject);
        ...
        // then try kml
        if (!removedIt) {
            oldDoc = overlay.docs[data.featureId];
            if (oldDoc) {
                oldDoc.remove();
    ...
    function showFeature(data, sender, msg) {
        var overlay = overlays[data.overlayId],
            ...
            doc;
        if (!overlay) {
            jc2cui.mapWidget.error({ sender: sender ? sender.id : '', type: 'map.feature.show', msg: msg, error: 'Unable to find overlay ' + data.overlayId || data.url });
            return;
        }

        // first try user drawn
        if (overlay.features[data.featureId]) {
            overlay.features[data.featureId].mapObject.show();
            if (drawingTools.is(":visible")) {
                makeFeatureEditable(overlay.features[data.featureId], true);
            }
            shown = true;
        }
    ...
    function hideFeature(data, sender, msg) {
        var overlay = overlays[data.overlayId],
            ...
            doc;
        if (!overlay) {
            jc2cui.mapWidget.error({ sender: sender ? sender.id : '', type: 'map.feature.hide', msg: msg, error: 'Unable to find overlay ' + data.overlayId || data.url });
            return;
        }

        // first try user drawn
        if (overlay.features[data.featureId]) {
            overlay.features[data.featureId].mapObject.hide();
            if (drawingTools.is(":visible")) {
                makeFeatureEditable(overlay.features[data.featureId], false);
            }
            if (jc2cui.mapWidget.infoWindow.getMapItem() && jc2cui.mapWidget.infoWindow.getMapItem().id === data.featureId) {
                jc2cui.mapWidget.infoWindow.close();
            }
    ...
    function selected(data) {
        var overlay = overlays[data.overlayId],
            ...
            bounds;
        if (!overlay) {
            console.log('maps V2 configured map widget unable to find overlay ' + data.overlayId + ' for selected item with id ' + data.selectedId + ' and name ' + data.selectedName);
            return;
        }
        ...
        if (selectedItem) {
            ...
            selectedItem = selectedItem.mapObject;
            switch (type) {
            case 'marker':
                loc = selectedItem.getLatLng();
                break;
            case 'line':
            case 'shape':
                ...
                break;
            default: // custom renderer - loc already set
                break;
            }
        } else {
        ...
        if (selectedItem) {
            if (loc) {
                ...
                console.log('maps V2 configured map widget unable to determine location of selected item with id ' + data.selectedId + ' and name ' + data.selectedName);
            }
        } else {
            console.log('maps V2 configured map widget unable to find selected item with id ' + data.selectedId + ' and name ' + data.selectedName);
        }
    }
    ...
    return {
        init: function (configuration) {
            ...
            /*if (!config) {
                jc2cui.mapWidget.showError('Missing config file. Unable to load Google Maps API');
                return;
            }

            if (config.ajaxLoadUrl) {
                $.ajax({
                    url: config.ajaxLoadUrl + '&callback=googleLoaded',
                    dataType: 'script',
            ...
            } else if (config.mapsApiUrl) {
                $.ajax({
                    url: config.mapsApiUrl + '&async=2&callback=mapsLoaded', // guessing that if direct api exists it will take async and callback as direct load of maps2 API from google does
                    dataType: 'script',
                    timeout: 20000, // At least as of jQuery 1.7.1 no longer getting error callbacks if cross domain script loading fails
                    error: function (XMLHttpRequest, textStatus, errorThrown) {
                        var msg = 'Error initializing Google Maps';
                ...
                jc2cui.mapWidget.showError('Config file includes neither a ajax load url or a direct Google Maps API url. Unable to load Google Maps API');
                return;
            }*/
            mapsLoaded(); // script loaded in html for now due to its usage of document.write
        },
        createOverlay: function (data, sender, msg) {
            ...
            if (data.parentId) {
                ...
                if (!parent) {
                    console.log("maps V2 configured map widget unable to find parent overlay with id " + data.parentId);
                }
            }
        ...
        checkResize: function () {
            ...
            setTimeout(function () { // or info window wont resize if we manually resized window
                ...
                $.each(overlays, function (index, overlay) {
                    $.each(overlay.docs, function (index, doc) {
                        ...
                        $.each(containers, function (index, container) {
                            if (container.markerOverlay) {
                                container.markerOverlay.doRedraw(true);
                            }
                        });
                    });
                });
        ...
        mapsReady: function () { // direct API load
            initMaps();
        },
        info: function () {
            ...
            return { name: 'Google Maps V2', type: '2D', entities: jc2cui.mapWidget.kmlParser.getEntitiesString(nonKmlEntities) };
        }
    };
}());