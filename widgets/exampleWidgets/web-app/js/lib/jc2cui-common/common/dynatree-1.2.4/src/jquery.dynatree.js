// Note: Applied patch as decribed at http://code.google.com/p/dynatree/issues/detail?id=252
// to remove usage of eval and replace with jQuery.parseJSON
// Also:
// - comment out fixes for issue 95 as they cause terible scrolling issues when checking unchecking nodes in IE 9
// - prevent calling remove on an unexpanded node from failing
// - Protect render with check for tree.bEnableUpdate since sometimes see render being called (for example sort and remove, easier then fixing each caller of render)
...
/* jsHint options*/
// Note: We currently allow eval() to parse the 'data' attribtes, when initializing from HTML. - Not true for JC2CUI version - see above
...
//			return a.data.title === b.data.title ? 0 : a.data.title > b.data.title ? 1 : -1;
		...
		if (this.tree.bEnableUpdate && deep !== "$norender$") {
			this.render();
		}
	},
...
//				self.logDebug("%o", title);
			...
			if( $li.attr("data") ) {
				...
				if( dataAttr ) {
					...
					try {
					    $.extend(data, $.parseJSON("(" + dataAttr + ")"));
					} catch(e) {
...
//		disabled: "dynatree-disabled",
	...
	debugLevel: 1, // 0:quiet, 1:normal, 2:debug $REPLACE:	debugLevel: 1,
