...
@implementation MNONSURLCacheManager
...
+ (NSString *)cachePathForImage:(NSString *)imageURL
{
    ...
    [cachesPath stringByAppendingPathComponent:
     [NSString stringWithFormat:@"%lx", (unsigned long)[imageURL hash]]];
}
...
+ (NSString *)documentsPathWithName:(NSString *)name
{
    ...
    return
     [docPath stringByAppendingPathComponent:
      [NSString stringWithFormat:@"%lx", (unsigned long)[name hash]]];
}
...
{ 
    if ([MNOProtocolManager isSupportedImageRequest:request]) {
        id response =  [MNONSURLCacheManager retrieveImageForRequest:request];
        if (response != nil)
            return response;
