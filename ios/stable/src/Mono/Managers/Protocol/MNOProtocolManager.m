...
static MNORouteRequestManager * requestRouter;


@interface MNOProtocolManager ()
...
{
    return [MNORouteRequestManager canRoute:request];
}

/**
 * Returns true if it's a hardware call, false otherwise.
 **/
+ (BOOL)canInitWithRequest:(NSURLRequest *)request
{
    if ([MNOProtocolManager isHardwareRequest:request]) {
        return YES;
...
- (NSString *)cachePathForRequest:(NSURLRequest *)aRequest
{
    ...
    [cachesPath stringByAppendingPathComponent:
     [NSString stringWithFormat:@"%x", (unsigned int)[[[aRequest URL] absoluteString] hash]]];
}
...
- (void) startLoading
{
    ...
    if ([MNOProtocolManager isHardwareRequest:request]) {
        [self sendResponseWith:request response:[requestRouter routeRequest:request]];
    }else if([MNOProtocolManager isSupportedImageRequest:request]){
